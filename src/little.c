...
static uint8_t lt_buffer_push(lt_VM *vm, lt_Buffer *buf, void *element) {
  ...
  memcpy((uint8_t *)buf->data + buf->element_size * buf->length, element,
         buf->element_size);
  buf->length++;
...
static void lt_buffer_pop(lt_Buffer *buf) { buf->length--; }
...
static lt_DebugInfo *_lt_get_debuginfo(lt_Object *obj) {
  ...
  case LT_OBJECT_CHUNK:
    return obj->u.chunk.debug;
  case LT_OBJECT_FN:
    return obj->u.fn.debug;
  case LT_OBJECT_CLOSURE:
    return LT_GET_OBJECT(obj->u.closure.function)->u.fn.debug;
  default:
...
lt_Value lt_make_string(lt_VM *vm, const char *string) {
  ...
  if (buf->element_size == 0)
    *buf = lt_buffer_new(sizeof(lt_StringDedupEntry));
  ...
  for (uint32_t i = 0; i < buf->length; i++) {
    ...
    if (entry->hash == hash) {
      return entry->value;
    } else if (entry->hash == 0 && first_empty == -1)
      first_empty = i;
  }
  ...
  if (first_empty != -1) {
    ...
    new_entry.value =
        (LT_NAN_MASK | LT_TYPE_STRING) | (bucket << 24) | (index & 0xFFFFFF);
...
const char *lt_get_string(lt_VM *vm, lt_Value value) {
  ...
  return ((lt_StringDedupEntry *)lt_buffer_at(vm->strings + bucket, index))
      ->string;
}
...
static void _lt_reference_string(lt_VM *vm, lt_Value value) {
  uint16_t bucket = (uint16_t)((value & 0xFFFFFF000000) >> 24);
  uint32_t index = value & 0xFFFFFF;
  ((lt_StringDedupEntry *)lt_buffer_at(vm->strings + bucket, index))
      ->refcount++;
}

static uint8_t faststrcmp(const char *a, uint64_t a_len, const char *b,
                          uint64_t b_len) {
  if (a_len != b_len)
    return 0;
...
#define PUSH_TOKEN(new_type)                                                   \
  {                                                                            \
    ...
    while (*current) {
      ...
      case '.':
        PUSH_TOKEN(LT_TOKEN_PERIOD) break;
      case ',':
        PUSH_TOKEN(LT_TOKEN_COMMA) break;
      case ':':
        PUSH_TOKEN(LT_TOKEN_COLON) break;
      case '(':
        PUSH_TOKEN(LT_TOKEN_OPENPAREN) break;
      case ')':
        PUSH_TOKEN(LT_TOKEN_CLOSEPAREN) break;
      case '[':
        PUSH_TOKEN(LT_TOKEN_OPENBRACKET) break;
      case ']':
        PUSH_TOKEN(LT_TOKEN_CLOSEBRACKET) break;
      case '{':
        PUSH_TOKEN(LT_TOKEN_OPENBRACE) break;
      case '}':
        PUSH_TOKEN(LT_TOKEN_CLOSEBRACE) break;
      case '+':
        PUSH_TOKEN(LT_TOKEN_PLUS) break;
      case '-':
        PUSH_TOKEN(LT_TOKEN_MINUS) break;
      case '*':
        PUSH_TOKEN(LT_TOKEN_MULTIPLY) break;
      case '/':
        PUSH_TOKEN(LT_TOKEN_DIVIDE) break;
      case '=':
        PUSH_TOKEN(LT_TOKEN_ASSIGN) break;
      }
      ...
      if (!found) {
        if (*current == '>') {
          ...
          newlit.u.string = vm->alloc(length + 1);
          strncpy(newlit.u.string, start, length);
          newlit.u.string[length] = 0;
          ...
          while ((isalnum(*current) && !isalpha(*current)) || *current == '.') {
            if (*current == '.') {
              if (has_decimal)
                _lt_tokenize_error(
                    vm, t.module, line, col,
                    "Can't have multiple decimals in number literal!");
              has_decimal = 1;
            }
		...
		lt_buffer_push(vm, &t.token_buffer, &_t); found = 1; }
          ...
          PUSH_STR_TOKEN("fn", LT_TOKEN_FN)
            ...
            if (!found) {
              ...
              strncpy(newid.name, start, length);
              newid.name[length] = 0;
          ...
          _lt_tokenize_error(vm, t.module, line, col, "Unrecognized token!");
      }
    }
...
lt_AstNode *_lt_get_node_of_type(lt_VM *vm, lt_Token *current, lt_Parser *p,
                                 lt_AstNodeType type) {
...
uint32_t _lt_tokens_equal(lt_Token *a, lt_Token *b) {
  return (a->type == LT_TOKEN_IDENTIFIER && b->type == LT_TOKEN_IDENTIFIER &&
          a->idx == b->idx);
}
...
uint16_t _lt_make_local(lt_VM *vm, lt_Scope *scope, lt_Token *t) {
  ...
  for (uint32_t i = 0; i < current->locals.length; ++i) {
    if (_lt_tokens_equal((lt_Token *)lt_buffer_at(&current->locals, i), t))
      return i;
  }
...
uint32_t _lt_find_local(lt_VM *vm, lt_Scope *scope, lt_Token *t) {
  ...
  for (uint32_t i = 0; i < current->upvals.length; ++i)
    if (_lt_tokens_equal((lt_Token *)lt_buffer_at(&current->upvals, i), t))
      return i | UPVAL_BIT;
...
lt_Scope *_lt_parse_block(lt_VM *vm, lt_Parser *p, lt_Token *start,
  ...
  if (makes_scope) {
    ...
    if (argnames)
      while (*argnames)
        _lt_make_local(vm, p->current, *argnames++);
  }
...
#define NEXT() (previous = current, current++)
  ...
  while (current->type != LT_TOKEN_END) {
    ...
    case LT_TOKEN_IF: {
      ...
      if (current->type != LT_TOKEN_OPENBRACE)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expeceted open brace to follow if expression!");
      current++;
      ...
      while (current->type == LT_TOKEN_ELSEIF ||
        ...
        if (current->type != LT_TOKEN_OPENBRACE)
          _lt_parse_error(vm, p->tkn->module, current,
                          "Expected open brace to follow else expression!");
        current++;
    ...
    case LT_TOKEN_FOR: {
      lt_AstNode *for_expr =
          _lt_get_node_of_type(vm, current, p, LT_AST_NODE_FOR);
      current++; // eat for

      lt_Token *ident = current++;
      uint16_t iteridx = _lt_make_local(vm, p->current, ident);

      if (current->type != LT_TOKEN_IN)
        _lt_parse_error(vm, p->tkn->module, current,
      ...
      strncpy(newid.name, FOR_ITER_NAME, len);
      newid.name[len] = 0;
      ...
      for_expr->u.loop.closureidx = _lt_make_local(vm, p->current, &tok);
      for_expr->u.loop.iterator = iter_expr;

      if (current->type != LT_TOKEN_OPENBRACE)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expected open brace to follow 'for' header!");
      current++;
    ...
    case LT_TOKEN_WHILE: {
      ...
      if (current->type != LT_TOKEN_OPENBRACE)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expected open brace to follow 'while' header!");
      current++;
    ...
    case LT_TOKEN_VAR: {
      lt_AstNode *declare =
          _lt_get_node_of_type(vm, current, p, LT_AST_NODE_DECLARE);
      current++;
      if (current->type == LT_TOKEN_IDENTIFIER) {
        declare->u.declare.identifier = current++;
        _lt_make_local(vm, p->current, declare->u.declare.identifier);
      } else
        _lt_parse_error(vm, p->tkn->module, current,
    ...
    default: {
      ...
      if (current->type == LT_TOKEN_ASSIGN) {
        ...
        result->u.assign.left = lhs;
        result->u.assign.right = expr;
      }
...
uint8_t _lt_get_prec(lt_TokenType op) {
  ...
  case LT_TOKEN_LT:
  case LT_TOKEN_LTE:
  case LT_TOKEN_EQUALS:
  case LT_TOKEN_NOTEQUALS:
    return 2;
  case LT_TOKEN_AND:
  case LT_TOKEN_OR:
  case LT_TOKEN_NOT:
...
#define PUSH_EXPR_FROM_OP(op)                                                  \
  if (op == LT_TOKEN_NOT || op == LT_TOKEN_NEGATE) {                           \
    lt_AstNode *unaryop =                                                      \
        _lt_get_node_of_type(vm, current, p, LT_AST_NODE_UNARYOP);             \
    unaryop->u.unary_op.type = op;                                             \
    lt_buffer_push(vm, &result, &unaryop);                                     \
  } else {                                                                     \
    lt_AstNode *binaryop =                                                     \
        _lt_get_node_of_type(vm, current, p, LT_AST_NODE_BINARYOP);            \
    binaryop->u.binary_op.type = op;                                           \
...
#define BREAK_ON_EXPR_BOUNDARY                                                 \
  ...
  while (current->type != LT_TOKEN_END) {
    switch (current->type) {
    case LT_TOKEN_IDENTIFIER: {
      BREAK_ON_EXPR_BOUNDARY

      lt_AstNode *ident =
          _lt_get_node_of_type(vm, current, p, LT_AST_NODE_IDENTIFIER);
      ident->u.identifier.token = NEXT();

      if (_lt_find_local(vm, p->current, ident->u.identifier.token) ==
          NOT_FOUND) {
      } // ERROR!
    ...
    case LT_TOKEN_OPENBRACKET: {
      ...
      if (is_index) {
        ...
        lt_AstNode *source = *(void **)lt_buffer_last(&result);
        lt_buffer_pop(&result);
        lt_AstNode *index =
            _lt_get_node_of_type(vm, current, p, LT_AST_NODE_INDEX);
        index->u.index.source = source;
        index->u.index.idx = idx_expr;
        lt_buffer_push(vm, &result, &index);
      } else {
        // array literal
        NEXT(); // eat bracket
        lt_AstNode *arr_expr =
            _lt_get_node_of_type(vm, current, p, LT_AST_NODE_ARRAY);
    ...
    case LT_TOKEN_PERIOD: {
      ...
      NEXT(); // eat period
      lt_AstNode *idx_expr =
          _lt_get_node_of_type(vm, current, p, LT_AST_NODE_LITERAL);
      if (current->type != LT_TOKEN_IDENTIFIER)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expected identifier to follow '.' operator!");
      idx_expr->u.literal.token = NEXT();
    ...
    case LT_TOKEN_OPENBRACE: {
      ...
      // any time we see this, assume it's a table lieral. all other braces
      // should be handled at block level
      lt_AstNode *table =
          _lt_get_node_of_type(vm, current, p, LT_AST_NODE_TABLE);
      table->u.table.keys = lt_buffer_new(sizeof(lt_AstNode *));
      table->u.table.values = lt_buffer_new(sizeof(lt_AstNode *));
      ...
      while (current->type != LT_TOKEN_CLOSEBRACE) {
        ...
        lt_buffer_push(vm, &table->u.table.values, &value);
      }
    ...
    case LT_TOKEN_FN: {
      ...
      if (current->type != LT_TOKEN_OPENPAREN)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expected open parenthesis to follow 'fn'!");
      current++;

      uint8_t nargs = 0;
      while (current->type == LT_TOKEN_IDENTIFIER) {
        func->u.fn.args[nargs++] = current++;
        if (current->type == LT_TOKEN_COMMA)
          current++;
      }

      if (current->type != LT_TOKEN_CLOSEPAREN)
        _lt_parse_error(
            vm, p->tkn->module, current,
            "Expecetd closing parenthesis to follow argument list!");
      current++;

      if (current->type != LT_TOKEN_OPENBRACE)
        _lt_parse_error(vm, p->tkn->module, current,
                        "Expected open brace to follow argument list!");
      current++;

      lt_Buffer body = lt_buffer_new(sizeof(lt_AstNode *));
      lt_Scope *fn_scope =
          _lt_parse_block(vm, p, current, &body, 1, 1, func->u.fn.args);
      current = fn_scope->end;

      func->u.fn.scope = fn_scope;
      func->u.fn.body = body;
...
expr_end:
  ...
  for (uint32_t i = 0; i < result.length; i++) {
    ...
    if (current->type == LT_AST_NODE_BINARYOP) {
      ...
      switch (current->u.binary_op.type) {
      case LT_TOKEN_LT:
      case LT_TOKEN_LTE:
        current->u.binary_op.type -= 2;
        current->u.binary_op.left = right;
        current->u.binary_op.right = left;
        break;
      default:
        current->u.binary_op.left = left;
        current->u.binary_op.right = right;
        break;
      }
  ...
  if (value_stack.length > 0 && out)
    *out = *(lt_AstNode **)lt_buffer_at(&value_stack, 0);
...
void lt_free(lt_VM *vm, uint32_t heapidx) {
  ...
  switch (obj->type) {
  case LT_OBJECT_CHUNK: {
    lt_buffer_destroy(vm, &obj->u.chunk.code);
    lt_buffer_destroy(vm, &obj->u.chunk.constants);
  } break;
  case LT_OBJECT_CLOSURE: {
  ...
  case LT_OBJECT_TABLE: {
    for (uint8_t i = 0; i < 16; ++i)
      lt_buffer_destroy(vm, obj->u.table.buckets + i);
  } break;
  case LT_OBJECT_ARRAY: {
    lt_buffer_destroy(vm, &obj->u.array);
  } break;
  case LT_OBJECT_NATIVEFN:
...
void lt_sweep_v(lt_VM *vm, lt_Value val) {
  if (LT_IS_OBJECT(val))
    lt_sweep(vm, LT_GET_OBJECT(val));
  else if (LT_IS_STRING(val))
    _lt_reference_string(vm, val);
}
...
void lt_sweep(lt_VM *vm, lt_Object *obj) {
  ...
  switch (obj->type) {
  case LT_OBJECT_CHUNK: {
    for (uint32_t i = 0; i < obj->u.chunk.constants.length; ++i) {
      lt_sweep_v(vm, *(lt_Value *)lt_buffer_at(&obj->u.chunk.constants, i));
    }
  } break;
  case LT_OBJECT_CLOSURE: {
    lt_sweep_v(vm, obj->u.closure.function);
  ...
  case LT_OBJECT_TABLE: {
    for (uint16_t i = 0; i < 16; ++i) {
      lt_Buffer *bucket = obj->u.table.buckets + i;
  ...
  case LT_OBJECT_ARRAY: {
    for (uint32_t j = 0; j < obj->u.array.length; ++j) {
      lt_sweep_v(vm, ((lt_TablePair *)lt_buffer_at(&obj->u.array, j))->key);
    }
  } break;
  default:
    // native functions and pointers don't have markbits or refcounts
    break;
  }
}
...
void lt_push(lt_VM *vm, lt_Value val) { (*vm->top++ = (val)); }

lt_Value lt_pop(lt_VM *vm) { return (*(--vm->top)); }

lt_Value lt_at(lt_VM *vm, uint32_t idx) { return *(vm->current->start + idx); }
...
uint16_t _lt_exec(lt_VM *vm, lt_Value callable, uint8_t argc) {
  ...
  switch (callee->type) {
  case LT_OBJECT_CHUNK: {
    frame->code = &callee->u.chunk.code;
    frame->constants = &callee->u.chunk.constants;
  } break;
  case LT_OBJECT_FN: {
    frame->code = &callee->u.fn.code;
    frame->constants = &callee->u.fn.constants;
  } break;
  case LT_OBJECT_CLOSURE: {
    lt_Object *fn = LT_GET_OBJECT(callee->u.closure.function);
    frame->upvals = &callee->u.closure.captures;
    if (fn->type == LT_OBJECT_FN) {
      frame->code = &fn->u.fn.code;
      frame->constants = &fn->u.fn.constants;
    } else {
...
inst_loop:
  ...
  case LT_OP_DUP:
    ...
    NEXT;
  case LT_OP_PUSHC:
    PUSH(*(lt_Value *)lt_buffer_at(frame->constants, ip->arg));
    NEXT;
  case LT_OP_PUSHN:
    PUSH(LT_VALUE_NULL);
    NEXT;
  ...
  case LT_OP_CLOSE: {
    ...
    closure->u.closure.captures = lt_buffer_new(sizeof(lt_Value));
    for (int i = 0; i < ip->arg; i++) {
      lt_buffer_push(vm, &closure->u.closure.captures, &POP());
    }
    closure->u.closure.function = POP();
    PUSH(LT_VALUE_OBJECT(closure));
  }
    NEXT;

  case LT_OP_CALL:
    _lt_exec(vm, POP(), (uint8_t)ip->arg);
    NEXT;
  ...
  default:
    lt_runtime_error(vm, "VM encountered unknown opcode!");
  }
...
#define OP(op)                                                                 \
  {                                                                            \
    lt_Op op = {LT_OP_##op, 0};                                                \
    lt_buffer_push(vm, code_body, &op);                                        \
    if (debug) {                                                               \
      lt_buffer_push(vm, debug, &node->loc);                                   \
    }                                                                          \
  }
#define OPARG(op, arg)                                                         \
  {                                                                            \
    lt_Op op = {LT_OP_##op, arg};                                              \
    lt_buffer_push(vm, code_body, &op);                                        \
    if (debug) {                                                               \
      lt_buffer_push(vm, debug, &node->loc);                                   \
    }                                                                          \
...
uint16_t _lt_push_constant(lt_VM *vm, lt_Buffer *constants, lt_Value constant) {
  for (uint32_t i = 0; i < constants->length; i++) {
    if ((*(lt_Value *)lt_buffer_at(constants, i)) == constant)
      return i;
  }
...
static void _lt_compile_body(lt_VM *vm, lt_Parser *p, const char *name,
                             lt_Buffer *debug, lt_Buffer *ast_body,
                             lt_Scope *scope, lt_Buffer *code_body,
                             lt_Buffer *constants);
static void _lt_compile_node(lt_VM *vm, lt_Parser *p, const char *name,
                             lt_Buffer *debug, lt_AstNode *node,
                             lt_Scope *scope, lt_Buffer *code_body,
                             lt_Buffer *constants);

static void _lt_compile_index(lt_VM *vm, lt_Parser *p, const char *name,
                              lt_Buffer *debug, lt_AstNode *node,
                              lt_Scope *scope, lt_Buffer *code_body,
                              lt_Buffer *constants) {
  _lt_compile_node(vm, p, name, debug, node->u.index.source, scope, code_body,
                   constants);
  _lt_compile_node(vm, p, name, debug, node->u.index.idx, scope, code_body,
                   constants);
}
...
static void _lt_compile_node(lt_VM *vm, lt_Parser *p, const char *name,
  ...
  case LT_AST_NODE_LITERAL: {
    lt_Token *t = node->u.literal.token;
    switch (t->type) {
    case LT_TOKEN_NULL_LITERAL:
      OP(PUSHN);
      break;
    ...
    case LT_TOKEN_STRING_LITERAL: {
      ...
      if (l->type == LT_TOKEN_NUMBER_LITERAL)
        con = LT_VALUE_NUMBER(l->u.number);
      else {
        con = lt_make_string(vm, l->u.string);
      }
  ...
  case LT_AST_NODE_TABLE: {
    ...
    for (int i = 0; i < size; ++i) {
      lt_AstNode *key = *(lt_AstNode **)lt_buffer_at(&node->u.table.keys, i);
      lt_AstNode *value =
          *(lt_AstNode **)lt_buffer_at(&node->u.table.values, i);
  ...
  case LT_AST_NODE_ARRAY: {
    ...
    for (int i = size - 1; i >= 0; --i) {
      lt_AstNode *value =
          *(lt_AstNode **)lt_buffer_at(&node->u.array.values, i);
      _lt_compile_node(vm, p, name, debug, value, scope, code_body, constants);
    }
  ...
  case LT_AST_NODE_IDENTIFIER: {
    uint32_t idx = _lt_find_local(vm, scope, node->u.identifier.token);
    if (idx == NOT_FOUND) {
      lt_Identifier *i = lt_buffer_at(&p->tkn->identifier_buffer,
                                      node->u.identifier.token->idx);
      lt_Value val = lt_make_string(vm, i->name);
      OPARG(PUSHC, _lt_push_constant(vm, constants, val));
      OP(GETG);
  ...
  case LT_AST_NODE_BINARYOP: {
    _lt_compile_node(vm, p, name, debug, node->u.binary_op.right, scope,
                     code_body, constants);
    _lt_compile_node(vm, p, name, debug, node->u.binary_op.left, scope,
                     code_body, constants);
    switch (node->u.binary_op.type) {
    case LT_TOKEN_PLUS:
      OP(ADD);
      break;
    case LT_TOKEN_MINUS:
  ...
  case LT_AST_NODE_UNARYOP: {
    _lt_compile_node(vm, p, name, debug, node->u.unary_op.expr, scope,
                     code_body, constants);
  ...
  case LT_AST_NODE_DECLARE: {
    uint16_t idx = _lt_make_local(vm, scope, node->u.declare.identifier);
    if (node->u.declare.expr) {
      _lt_compile_node(vm, p, name, debug, node->u.declare.expr, scope,
                       code_body, constants);
      OPARG(STORE, idx);
    }
  } break;
  ...
  case LT_AST_NODE_ASSIGN: {
    lt_AstNode *target = node->u.assign.left;
    if (target->type == LT_AST_NODE_IDENTIFIER) {
      _lt_compile_node(vm, p, name, debug, node->u.assign.right, scope,
                       code_body, constants);
      uint32_t idx = _lt_find_local(vm, scope, target->u.identifier.token);
      if (idx == NOT_FOUND)
        _lt_parse_error(vm, name, target->u.identifier.token,
                        "Can't find local to assign to!");
      else if ((idx & UPVAL_BIT) == UPVAL_BIT)
        OPARG(STOREUP, idx & 0xFFFF)
      else
        OPARG(STORE, idx & 0xFFFF);
    } else if (target->type == LT_AST_NODE_INDEX) {
      _lt_compile_index(vm, p, name, debug, target, scope, code_body,
                        constants);
      _lt_compile_node(vm, p, name, debug, node->u.assign.right, scope,
                       code_body, constants);
      OP(SETT);
  ...
  case LT_AST_NODE_FN: {
    ...
    fn->u.fn.arity = narg;
    fn->u.fn.code = lt_buffer_new(sizeof(lt_Op));
    fn->u.fn.constants = lt_buffer_new(sizeof(lt_Value));
    if (vm->generate_debug) {
      fn->u.fn.debug = vm->alloc(sizeof(lt_DebugInfo));
      fn->u.fn.debug->locations = lt_buffer_new(sizeof(lt_DebugLoc));
    ...
    if (node->u.fn.scope->upvals.length > 0) {
      lt_Buffer *upvals = &node->u.fn.scope->upvals;
      // this is actually a closure

      for (int i = upvals->length - 1; i >= 0; i--) {
        uint32_t idx =
            _lt_find_local(vm, scope, (lt_Token *)lt_buffer_at(upvals, i));
  ...
  case LT_AST_NODE_CALL: {
    ...
    _lt_compile_node(vm, p, name, debug, node->u.call.callee, scope, code_body,
                     constants);
    OPARG(CALL, narg);
  } break;
  ...
  case LT_AST_NODE_RETURN: {
    if (node->u.ret.expr) {
      _lt_compile_node(vm, p, name, debug, node->u.ret.expr, scope, code_body,
                       constants);
      OPARG(RET, 1);
    } else
      OP(RET);
...
#define REG_JMP()                                                              \
  branch_stack[n_branches++] = code_body->length;                              \
  OP(NOP);
  ...
  case LT_AST_NODE_IF: {
    ...
    _lt_compile_node(vm, p, name, debug, node->u.branch.expr, scope, code_body,
                     constants);
    uint32_t jidx = code_body->length;
    OP(JMPC);

    _lt_compile_body(vm, p, name, debug, &node->u.branch.body, scope, code_body,
                     constants);
    REG_JMP();

    ((lt_Op *)lt_buffer_at(code_body, jidx))->arg =
        code_body->length - jidx - 1;

    uint8_t has_elseif = 0, has_else = 0;

    lt_AstNode *next = node->u.branch.next;
    while (next) {
      if (next->type == LT_AST_NODE_ELSEIF) {
        has_elseif = 1;

        if (has_else)
          lt_error(vm, "'else' must be last in if-chain!");

        _lt_compile_node(vm, p, name, debug, next->u.branch.expr, scope,
                         code_body, constants);
        uint32_t jidx = code_body->length;
        OP(JMPC);

        _lt_compile_body(vm, p, name, debug, &next->u.branch.body, scope,
                         code_body, constants);
        REG_JMP();
      ...
      next = next->u.branch.next;
    }

    if (has_elseif || has_else) {
      for (uint32_t i = 0; i < n_branches; i++) {
        uint32_t loc = branch_stack[i];
        *((lt_Op *)lt_buffer_at(code_body, loc)) =
            (lt_Op){LT_OP_JMP, code_body->length - loc - 1};
      }
    }
  } break;

  case LT_AST_NODE_FOR: {
    _lt_compile_node(vm, p, name, debug, node->u.loop.iterator, scope,
                     code_body, constants);
    OPARG(STORE, node->u.loop.closureidx);

    uint32_t loop_header = code_body->length;
    OPARG(LOAD, node->u.loop.closureidx);
    OPARG(CALL, 0);
    OPARG(STORE, node->u.loop.identifier);
    OPARG(LOAD, node->u.loop.identifier);
    uint32_t loop_start = code_body->length;
    OPARG(JMPN, 0);

    _lt_compile_body(vm, p, name, debug, &node->u.loop.body, scope, code_body,
                     constants);
...
lt_Value lt_compile(lt_VM *vm, lt_Parser *p) {
  ...
  chunk->u.chunk.code = lt_buffer_new(sizeof(lt_Op));
  chunk->u.chunk.constants = lt_buffer_new(sizeof(lt_Value));

  if (p->tkn->module) {
    uint32_t len = (uint32_t)strlen(p->tkn->module);
    chunk->u.chunk.name = vm->alloc(len + 1);
    memcpy(chunk->u.chunk.name, p->tkn->module, len);
    chunk->u.chunk.name[len] = 0;
  }

  if (vm->generate_debug) {
    chunk->u.chunk.debug = vm->alloc(sizeof(lt_DebugInfo));
    chunk->u.chunk.debug->locations = lt_buffer_new(sizeof(lt_DebugLoc));
...
void lt_free_parser(lt_VM *vm, lt_Parser *p) {
  for (uint32_t i = 0; i < p->ast_nodes.length; i++) {
    ...
    case LT_AST_NODE_CHUNK:
      lt_buffer_destroy(vm, &entry->u.chunk.body);
      lt_free_scope(vm, entry->u.chunk.scope);
      break;
    case LT_AST_NODE_TABLE:
      lt_buffer_destroy(vm, &entry->u.table.keys);
      lt_buffer_destroy(vm, &entry->u.table.values);
      break;
    case LT_AST_NODE_FN: /*lt_buffer_destroy(vm, &entry->fn.body);*/
      lt_free_scope(vm, entry->u.fn.scope);
      break;
    case LT_AST_NODE_IF:
    case LT_AST_NODE_ELSEIF:
    case LT_AST_NODE_ELSE:
      lt_buffer_destroy(vm, &entry->u.branch.body);
      break;
    default:
      // do I need to cover while and for bodies?
      break;
    }
...
lt_TablePair *_lt_table_index(lt_VM *vm, lt_Value table, lt_Value key,
                              uint8_t alloc) {
  (void)vm;
  uint8_t bucket = HASH(key);
  lt_Buffer *buf = LT_GET_OBJECT(table)->u.table.buckets + bucket;
  if (alloc && buf->element_size == 0)
    *buf = lt_buffer_new(sizeof(lt_TablePair));
...
lt_Value lt_array_push(lt_VM *vm, lt_Value array, lt_Value val) {
  ...
  if (arr->u.array.element_size == 0)
    arr->u.array = lt_buffer_new(sizeof(lt_Value));
  lt_buffer_push(vm, &arr->u.array, &val);
  return val;
}
...
lt_Value lt_array_remove(lt_VM *vm, lt_Value array, int32_t idx) {
  ...
  if (!LT_IS_ARRAY(array))
    return LT_VALUE_NULL;
  lt_Object *arr = LT_GET_OBJECT(array);
  lt_Value old = *(lt_Value *)lt_buffer_at(&arr->u.array, idx);
  lt_buffer_cycle(&arr->u.array, idx);
  return old;
}
