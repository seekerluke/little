...
char *ltstd_tostring(lt_VM *vm, lt_Value val) {
  ...
  if (LT_IS_TRUE(val))
    len = snprintf(scratch, 256, "true");
  if (LT_IS_FALSE(val))
    len = snprintf(scratch, 256, "false");
  if (LT_IS_STRING(val))
    len = snprintf(scratch, 256, "%s", lt_get_string(vm, val));
  ;
  ...
  if (LT_IS_OBJECT(val)) {
    ...
    case LT_OBJECT_CHUNK:
      len = snprintf(scratch, 256, "chunk 0x%lx", (uintptr_t)obj);
      break;
    case LT_OBJECT_CLOSURE:
      len = snprintf(scratch, 256, "closure 0x%lx | %d upvals",
    ...
    case LT_OBJECT_TABLE:
      len = snprintf(scratch, 256, "table 0x%lx", (uintptr_t)obj);
      break;
    case LT_OBJECT_ARRAY:
      len = snprintf(scratch, 256, "array | %d", lt_array_length(val));
      break;
    case LT_OBJECT_NATIVEFN:
      len = snprintf(scratch, 256, "native %p", (void *)obj->u.native);
      break;
    case LT_OBJECT_PTR:
      len = snprintf(scratch, 256, "ptr %p", (void *)obj->u.ptr);
      break;
    }
  }
...
static uint8_t _lt_clock(lt_VM *vm, uint8_t argc) {
  if (argc != 0)
    lt_runtime_error(vm, "Expected no arguments to io.clock!");
...
static uint8_t _lt_require(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected path argument to io.require!");
...
#define LT_SIMPLE_MATH_FN(name)                                                \
  static uint8_t _lt_##name(lt_VM *vm, uint8_t argc) {                         \
    if (argc != 1)                                                             \
      lt_runtime_error(vm, "Expected one argument to math." #name "!");        \
...
static uint8_t _lt_array_next(lt_VM *vm, uint8_t argc) {
  ...
  lt_Value to_return =
      idx >= lt_array_length(arr) ? LT_VALUE_NULL : *lt_array_at(arr, idx);
...
static uint8_t _lt_array_each(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to array.each!");

  lt_Value arr = lt_pop(vm);

  if (!LT_IS_ARRAY(arr))
    lt_runtime_error(vm, "Expected argument to array.each to be array!");
...
static uint8_t _lt_range_iter(lt_VM *vm, uint8_t argc) {
  ...
  if (lt_get_number(start) >= lt_get_number(end)) {
    lt_push(vm, LT_VALUE_NULL);
    return 1;
  }

  lt_setupval(vm, 2,
              lt_make_number(lt_get_number(start) + lt_get_number(step)));
...
static uint8_t _lt_range(lt_VM *vm, uint8_t argc) {
  ...
  if (argc == 1) {
    ...
    lt_runtime_error(
        vm, "Expected 1-3 args for array.range([start,] end [, step]!");

  if (!LT_IS_NUMBER(start) || !LT_IS_NUMBER(end) || !LT_IS_NUMBER(step))
    lt_runtime_error(vm,
                     "Expected all arguments to array.range to be numbers!");
...
static uint8_t _lt_array_len(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to array.len!");
  lt_Value arr = lt_pop(vm);
  if (!LT_IS_ARRAY(arr))
    lt_runtime_error(vm, "Expected argument to array.len to be array!");
...
static uint8_t _lt_array_pop(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to array.pop!");
  lt_Value arr = lt_pop(vm);
  if (!LT_IS_ARRAY(arr))
    lt_runtime_error(vm, "Expected argument to array.pop to be array!");
...
static uint8_t _lt_array_last(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to array.last!");
  lt_Value arr = lt_pop(vm);
  if (!LT_IS_ARRAY(arr))
    lt_runtime_error(vm, "Expected argument to array.last to be array!");
...
static uint8_t _lt_array_remove(lt_VM *vm, uint8_t argc) {
  ...
  if (!LT_IS_NUMBER(idx))
    lt_runtime_error(vm,
                     "Expected second argument to array.remove to be number!");
...
static uint8_t _lt_gc_collect(lt_VM *vm, uint8_t argc) {
  if (argc != 0)
    lt_runtime_error(vm, "Expected no arguments to gc.collect!");
...
static uint8_t _lt_gc_addroot(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to gc.addroot!");
  lt_Value val = lt_pop(vm);
  if (!LT_IS_OBJECT(val))
    lt_runtime_error(vm, "Expected argument to gc.addroot to be object!");
  lt_Object *obj = LT_GET_OBJECT(val);
  lt_nocollect(vm, obj);
  return 0;
}
...
static uint8_t _lt_gc_removeroot(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to gc.removeroot!");
  lt_Value val = lt_pop(vm);
  if (!LT_IS_OBJECT(val))
    lt_runtime_error(vm, "Expected argument to gc.removeroot to be object!");
  lt_Object *obj = LT_GET_OBJECT(val);
...
static uint8_t _lt_string_from(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to string.from!");
  lt_Value val = lt_pop(vm);
...
static uint8_t _lt_string_concat(lt_VM *vm, uint8_t argc) {
  if (argc < 2)
    lt_runtime_error(vm, "Expected at least two arguments to string.concat!");
  ...
  for (int32_t i = argc - 1; i >= 0; --i) {
    ...
    if (!LT_IS_STRING(val))
      lt_runtime_error(vm, "Non-string argument to string.concat!");
    uint32_t oldlen = len;
...
static uint8_t _lt_string_len(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to string.len!");
  lt_Value val = lt_pop(vm);
  if (!LT_IS_STRING(val))
    lt_runtime_error(vm, "Non-string argument to string.len!");
...
static uint8_t _lt_string_sub(lt_VM *vm, uint8_t argc) {
  if (argc < 2)
    lt_runtime_error(vm, "Expected at least two arguments to string.sub!");
  ...
  if (!LT_IS_STRING(str))
    lt_runtime_error(vm, "Non-string argument to string.sub!");
  if (!LT_IS_NUMBER(start))
...
static uint8_t _lt_string_format(lt_VM *vm, uint8_t argc) {
  if (argc < 1)
    lt_runtime_error(vm,
                     "Expected at least a template string to string.format!");
  lt_Value val = *(vm->top - argc);
  if (!LT_IS_STRING(val))
    lt_runtime_error(vm, "Non-string argument to string.format!");
    ...
    if (*format == '%') {
      ...
      scan_format:
        ...
        case 's': {
          ...
          fmtbuf[fmtloc] = 0;
          o_idx +=
              snprintf(output + o_idx, 1024 - o_idx, fmtbuf,
                       lt_get_string(vm, *(vm->top - argc + current_arg++)));
        } break;
        default:
          fmtbuf[fmtloc++] = *format++;
          goto scan_format;
...
static uint8_t _lt_string_typeof(lt_VM *vm, uint8_t argc) {
  if (argc != 1)
    lt_runtime_error(vm, "Expected one argument to string.typeof!");
  lt_Value val = lt_pop(vm);
  if (LT_IS_NULL(val))
    lt_push(vm, lt_make_string(vm, "null"));
  else if (LT_IS_NUMBER(val))
    lt_push(vm, lt_make_string(vm, "number"));
  else if (LT_IS_BOOL(val))
    lt_push(vm, lt_make_string(vm, "boolean"));
  else if (LT_IS_STRING(val))
    lt_push(vm, lt_make_string(vm, "string"));
  else if (LT_IS_FUNCTION(val))
    lt_push(vm, lt_make_string(vm, "function"));
  else if (LT_IS_CLOSURE(val))
    lt_push(vm, lt_make_string(vm, "closure"));
  else if (LT_IS_ARRAY(val))
    lt_push(vm, lt_make_string(vm, "array"));
  else if (LT_IS_TABLE(val))
    lt_push(vm, lt_make_string(vm, "table"));
  else if (LT_IS_NATIVE(val))
...
void ltstd_open_io(lt_VM *vm) {
  ...
  lt_table_set(vm, t, lt_make_string(vm, "print"),
               lt_make_native(vm, _lt_print));
  lt_table_set(vm, t, lt_make_string(vm, "clock"),
               lt_make_native(vm, _lt_clock));
  lt_table_set(vm, t, lt_make_string(vm, "require"),
               lt_make_native(vm, _lt_require));
...
void ltstd_open_math(lt_VM *vm) {
  ...
  lt_table_set(vm, t, lt_make_string(vm, "floor"),
               lt_make_native(vm, _lt_floor));
  lt_table_set(vm, t, lt_make_string(vm, "ceil"), lt_make_native(vm, _lt_ceil));
  lt_table_set(vm, t, lt_make_string(vm, "round"),
               lt_make_native(vm, _lt_round));
  ...
  lt_table_set(vm, t, lt_make_string(vm, "log10"),
               lt_make_native(vm, _lt_log10));
  ...
  lt_table_set(vm, t, lt_make_string(vm, "pi"),
               LT_VALUE_NUMBER(3.14159265358979323846));
  lt_table_set(vm, t, lt_make_string(vm, "e"),
               LT_VALUE_NUMBER(2.71828182845904523536));
...
void ltstd_open_array(lt_VM *vm) {
  lt_Value t = lt_make_table(vm);
  lt_table_set(vm, t, lt_make_string(vm, "each"),
               lt_make_native(vm, _lt_array_each));
  lt_table_set(vm, t, lt_make_string(vm, "range"),
               lt_make_native(vm, _lt_range));

  lt_table_set(vm, t, lt_make_string(vm, "len"),
               lt_make_native(vm, _lt_array_len));
  lt_table_set(vm, t, lt_make_string(vm, "last"),
               lt_make_native(vm, _lt_array_last));
  lt_table_set(vm, t, lt_make_string(vm, "pop"),
               lt_make_native(vm, _lt_array_pop));
  lt_table_set(vm, t, lt_make_string(vm, "push"),
               lt_make_native(vm, _lt_array_push));
  lt_table_set(vm, t, lt_make_string(vm, "remove"),
               lt_make_native(vm, _lt_array_remove));
...
void ltstd_open_string(lt_VM *vm) {
  ...
  lt_table_set(vm, t, lt_make_string(vm, "from"),
               lt_make_native(vm, _lt_string_from));
  lt_table_set(vm, t, lt_make_string(vm, "concat"),
               lt_make_native(vm, _lt_string_concat));
  lt_table_set(vm, t, lt_make_string(vm, "len"),
               lt_make_native(vm, _lt_string_len));
  lt_table_set(vm, t, lt_make_string(vm, "sub"),
               lt_make_native(vm, _lt_string_sub));
  lt_table_set(vm, t, lt_make_string(vm, "format"),
               lt_make_native(vm, _lt_string_format));
  lt_table_set(vm, t, lt_make_string(vm, "typeof"),
               lt_make_native(vm, _lt_string_typeof));
...
void ltstd_open_gc(lt_VM *vm) {
  ...
  lt_table_set(vm, t, lt_make_string(vm, "collect"),
               lt_make_native(vm, _lt_gc_collect));
  lt_table_set(vm, t, lt_make_string(vm, "addroot"),
               lt_make_native(vm, _lt_gc_addroot));
  lt_table_set(vm, t, lt_make_string(vm, "removeroot"),
               lt_make_native(vm, _lt_gc_removeroot));
